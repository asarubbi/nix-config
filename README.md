...
What’s included:
- Shared, named package profiles in `modules/common/packages.nix` (`portable`, `dev`, `desktop`, `devbox`, `titan`) reused everywhere.
...
- Portable flake in `flakes/portable` exporting a minimal CLI dev shell for macOS/Linux/WSL.
...
- Portable macOS/Linux/WSL: `nix develop ./flakes/portable` (default shell) or `nix develop .#portable` (root flake output)
